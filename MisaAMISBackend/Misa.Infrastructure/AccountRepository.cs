    ...
    public class AccountRepository : BaseRepository<Account>, IAccountRepository
    {
        ...
        public object getAccountPagingFilter(string searchData, int pageIndex, int pageSize, string type)
        {
            using (_dbConnection = new NpgsqlConnection(_connectionString))
            {
                ...
                var typeAccount = type == null ? string.Empty : type;
                ...
                dynamicParameters.Add("@type_account", typeAccount);

                var sql = "select * from  public.func_get_account_paging_filter(@search_data, @type_account) limit @page_size offset @offset;";
                sql += "select count(*) from (select * from  public.func_get_account_paging_filter(@search_data, @type_account)) as filtertable;";
